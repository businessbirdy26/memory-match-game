/* Vendored from node_modules/@capacitor-community/admob/dist/plugin.js — this project has
   no JS bundler, so the plugin is included as a plain <script> instead of an ES import.
   Registers itself as window.Capacitor.Plugins.AdMob once loaded after capacitor.js.
   (The global var name below, "capacitorStripe", is upstream's own build output — unrelated
   to this file's contents, which is AdMob, not Stripe. To update: bump
   @capacitor-community/admob in package.json, then re-copy that file over this one.) */
